...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if ( setupfinished==true) MakeClientZombie2(client);
...
stock SetLargeHealth(client)
{
    ...
    if ( GetConVarInt(cv_Debug) & DEBUG_HEALTH == DEBUG_HEALTH ) LogMessage("%N's max health: %d", client, newHealth);
        // Update the client's max health value.
...
stock Float:CalculateZombieHealthMultiplier()
{
    ...
    for ( new i = 1; i <= MaxClients; i++ )
    {
        if ( IsClientInGame(i) && GetClientTeam(i) == TEAM_BLUE && IsPlayerAlive(i) ) bluCount++;
    }  
    ...
    new Float:multiplier = Remap(float(redCount/bluCount), zMinPl, zMaxPl, zMin, zMax);
